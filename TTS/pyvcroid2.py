...
class VcRoid2(object):
    ...
    def __init__(self, *, install_path=None, install_path_x86=None):
        ...
        if install_path is None:
            if 2 ** 32 <= sys.maxsize:
                # Find 64bit DLL
                rfid = c_char_p(
                    b"\x77\x93\x80\x6D\xF0\x6A\x4B\x44\x89\x57\xA3\x77\x3F\x02\x20\x0E"
                )
            else:
                # Find 32bit DLL
                rfid = c_char_p(
                    b"\xEF\x40\x5A\x7C\xFB\xA0\xFC\x4B\x87\x4A\xC0\xF2\xE0\xB9\xFA\x8E"
                )
            pwstr = c_wchar_p()
            windll.shell32.SHGetKnownFolderPath(
                rfid, c_uint32(0), c_void_p(), byref(pwstr)
            )
        ...
        self.__dll.AITalkAPI_TextToKana.argtypes = [
            POINTER(c_int32),
            POINTER(aitalk.TJobParam),
            c_char_p,
        ]
        ...
        self.__dll.AITalkAPI_TextToSpeech.argtypes = [
            POINTER(c_int32),
            POINTER(aitalk.TJobParam),
            c_char_p,
        ]
    ...
    def loadLanguage(self, language_name):
        ...
        if (result != aitalk.ResultCode.SUCCESS) and (
            result != aitalk.ResultCode.NOT_LOADED
        ):
            raise Exception(result)

        # Load new language library
        language_path = c_char_p(
            (self.__install_path_x86 + "\\Lang\\" + language_name).encode("shift-jis")
        )
        cd = os.getcwd()
        try:
    ...
    def reloadPhraseDictionary(self, path):
        ...
        if path is None:
            return
        result = self.__dll.AITalkAPI_ReloadPhraseDic(
            c_char_p(path.encode("shift-jis"))
        )
    ...
    def reloadSymbolDictionary(self, path):
        ...
        if path is None:
            return
        result = self.__dll.AITalkAPI_ReloadSymbolDic(
            c_char_p(path.encode("shift-jis"))
        )
        if result == aitalk.ResultCode.USERDIC_NOENTRY:
            self.__dll.AITalkAPI_ReloadSymbolDic(c_void_p())
    ...
    def loadVoice(self, voice_name):
        ...
        result = self.__dll.AITalkAPI_VoiceLoad(
            c_char_p(voice_name.encode("shift-jis"))
        )
        if result != aitalk.ResultCode.SUCCESS:
            raise Exception(result)
        ...
        if result != aitalk.ResultCode.INSUFFICIENT:
            raise Exception(result)
        speaker_count = (param_size.value - sizeof(aitalk.createTtsParam(0))) // sizeof(
            aitalk.TSpeakerParam
        )
        ...
        result = self.__dll.AITalkAPI_GetParam(
            self.__default_parameter, byref(param_size)
        )
        if result != aitalk.ResultCode.SUCCESS:
            raise Exception(result)
        ...
        memmove(
            addressof(self.__parameter),
            addressof(self.__default_parameter),
            sizeof(TTtsParam),
        )
        ...
        self.__parameter.extendFormat = (
            aitalk.ExtendFormat.JEITA_RUBY | aitalk.ExtendFormat.AUTO_BOOKMARK
        )
    ...
    def textToKana(self, text, *, timeout=None):
        ...
        output = bytearray()
        text_buf = (
            c_char * min(self.__parameter.lenTextBufBytes, VcRoid2.__LEN_TEXT_BUF_MAX)
        )()
        ...
        def callback(reason_code, job_id, user_data):
            ...
            if (
                (reason != aitalk.EventReasonCode.TEXTBUF_FULL)
                and (reason != aitalk.EventReasonCode.TEXTBUF_FLUSH)
                and (reason != aitalk.EventReasonCode.TEXTBUF_CLOSE)
            ):
                return 0
            while True:
                bytes_read = c_uint32()
                position = c_uint32()
                result = self.__dll.AITalkAPI_GetKana(
                    c_int32(job_id),
                    text_buf,
                    c_uint32(sizeof(text_buf)),
                    byref(bytes_read),
                    byref(position),
                )
                if result != aitalk.ResultCode.SUCCESS:
        ...
        try:
            ...
            job_id = c_int32()
            job_param = aitalk.TJobParam(
                c_uint32(int(aitalk.JobInOut.PLAIN_TO_AIKANA)), c_void_p()
            )
            (
                shiftjis_string,
                shiftjis_positions,
            ) = VcRoid2.__CalculateShiftJisCharaterPositions(text)
            result = self.__dll.AITalkAPI_TextToKana(
                byref(job_id), job_param, c_char_p(shiftjis_string)
            )
            if result != aitalk.ResultCode.SUCCESS:
                raise Exception(result)
    ...
    def kanaToSpeech(self, kana, *, timeout=None, raw=False):
        ...
        event = threading.Event()
        raw_buf = (
            c_char * min(self.__parameter.lenRawBufBytes * 2, VcRoid2.__LEN_RAW_BUF_MAX)
        )()
        output = io.BytesIO()
        if not raw:
        ...
        def rawbuf_callback(reason_code, job_id, tick, user_data):
            reason = aitalk.EventReasonCode(reason_code)
            if (
                (reason != aitalk.EventReasonCode.RAWBUF_FULL)
                and (reason != aitalk.EventReasonCode.RAWBUF_FLUSH)
                and (reason != aitalk.EventReasonCode.RAWBUF_CLOSE)
            ):
                return 0
            while True:
                samples_read = c_uint32()
                result = self.__dll.AITalkAPI_GetData(
                    c_int32(job_id),
                    raw_buf,
                    c_uint32(sizeof(raw_buf) // 2),
                    byref(samples_read),
                )
                if result != aitalk.ResultCode.SUCCESS:
                    break
        ...
        try:
            ...
            job_id = c_int32()
            job_param = aitalk.TJobParam(
                c_uint32(int(aitalk.JobInOut.AIKANA_TO_WAVE)), c_void_p()
            )
            result = self.__dll.AITalkAPI_TextToSpeech(
                byref(job_id), job_param, c_char_p(kana.encode("shift-jis"))
            )
            if result != aitalk.ResultCode.SUCCESS:
                raise Exception(result)
...
class Param(object):
    def __init__(self, default_parameter, parameter):
        ...
        for index in range(
            min(self.__parameter.numSpeakers, len(self.__parameter.speaker))
        ):
            speaker_param = self.__parameter.speaker[index]
            if speaker_param.voiceName == self.__parameter.voiceName:
                self.__default_speaker_parameter = self.__default_parameter.speaker[
                    index
                ]
    ...
    def masterVolume(self, value):
        ...
        Current master volume (マスター音量) : float
        '''
        self.__parameter.volume = max(
            self.minMasterVolume, min(float(value), self.maxMasterVolume)
        )
    ...
    def volume(self, value):
        ...
        Current volume (音量) : float
        '''
        self.__speaker_parameter.volume = max(
            self.minVolume, min(float(value), self.maxVolume)
        )
    ...
    def speed(self, value):
        ...
        Current speed (話速) : float
        '''
        self.__speaker_parameter.speed = max(
            self.minSpeed, min(float(value), self.maxSpeed)
        )
    ...
    def pitch(self, value):
        ...
        Current pitch (高さ) : float
        '''
        self.__speaker_parameter.pitch = max(
            self.minPitch, min(float(value), self.maxPitch)
        )
    ...
    def emphasis(self, value):
        ...
        Current emphasis (抑揚) : float
        '''
        self.__speaker_parameter.range = max(
            self.minEmphasis, min(float(value), self.maxEmphasis)
        )
    ...
    def pauseMiddle(self, value):
        ...
        Current middle pause (短ポーズ時間) [ms] : int
        '''
        self.__speaker_parameter.pauseMiddle = max(
            self.minPauseMiddle, min(int(value), self.maxPauseMiddle)
        )
    ...
    def pauseLong(self, value):
        ...
        pauseLong must be longer than or equal to pauseMiddle
        '''
        self.__speaker_parameter.pauseLong = max(
            self.minPauseLong, min(int(value), self.maxPauseLong)
        )
    ...
    def pauseSentence(self, value):
        ...
        pauseSentence must be longer than or equal to pauseLong
        '''
        self.__speaker_parameter.pauseSentence = max(
            self.minPauseSentence, min(int(value), self.maxPauseSentence)
        )
